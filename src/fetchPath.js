...
function configureProfile({ considerElevation, downhillCost, uphillCost }) {
  return `
...
assign consider_elevation = ${considerElevation}  # %consider_elevation% | Set to false to ignore elevation in routing | boolean
assign downhillcost       = ${downhillCost}       # %downhillcost% | Cost for going downhill | number
assign downhillcutoff     = 1.5                   # %downhillcutoff% | Gradients below this value in percents are not counted. | number
assign uphillcost         = ${uphillCost}         # %uphillcost% | Cost for going uphill | number
...
async function fetchProfile(options) {
  const text = configureProfile(options);

  const { profileid } = await wretch("https://brouter.de/brouter/profile").body(text).post().json();

  return profileid;
}

export default async function fetchPath(startCoords, endCoords, options) {
  const profileId = await fetchProfile(options);

  const url = new URL("https://brouter.de/brouter");

  [
    ["lonlats", `${startCoords.lon},${startCoords.lat}|${endCoords.lon},${endCoords.lat}`],
    ["profile", profileId],
    ["alternativeidx", "0"],
    ["format", "geojson"],
  ].forEach(([k, v]) => url.searchParams.append(k, v));

  const { features } = await wretch(url.toString()).get().json();
  const feature = features[0];
  const path = feature.geometry.coordinates;

  const properties = feature.properties;
  const distance = parseFloat(properties["track-length"]);
  const elevationChange = parseFloat(properties["plain-ascend"]);

  return { path, distance, elevationChange };
}
